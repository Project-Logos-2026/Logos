...
TAG_DICTIONARY: dict[str, dict] = {
    "axiomatic":        {"definition": "Invariant logical premise applied unconditionally", "category": "structural"},
    "constraint":       {"definition": "Precondition or bound that limits execution space", "category": "structural"},
    "validation":       {"definition": "Pass/fail gate over a data or state assertion", "category": "structural"},
    "semantic":         {"definition": "Relates to meaning, ontology, or token interpretation", "category": "domain"},
    "reasoning":        {"definition": "Inference, deduction, or logical derivation capability", "category": "domain"},
    "planning":         {"definition": "Goal decomposition and task sequencing capability", "category": "domain"},
    "agent_runtime":    {"definition": "Controls or participates in agent lifecycle execution", "category": "domain"},
    "safety_guard":     {"definition": "Enforces fail-closed or governance-safe execution", "category": "domain"},
    "memory_access":    {"definition": "Reads or writes to a persistent state or knowledge store", "category": "domain"},
    "tool_interface":   {"definition": "Integrates with or wraps an external tool or LLM provider", "category": "domain"},
    "learning":         {"definition": "Adapts behavior from observations or feedback signals", "category": "domain"},
    "numeric":          {"definition": "Performs deterministic mathematical computation", "category": "domain"},
    "symbolic":         {"definition": "Operates over symbolic or formal logic representations", "category": "domain"},
    "probabilistic":    {"definition": "Incorporates stochastic or Bayesian inference", "category": "domain"},
    "protocol":         {"definition": "Defines or enforces a structured communication protocol", "category": "structural"},
    "orchestration":    {"definition": "Coordinates sequencing of multiple system components", "category": "structural"},
    "bootstrap":        {"definition": "Involved in system startup or initialization", "category": "structural"},
    "identity":         {"definition": "Manages or validates agent or resource identity", "category": "structural"},
    "bijective":        {"definition": "Implements a one-to-one deterministic mapping", "category": "domain"},
    "temporal":         {"definition": "Handles time-based ordering, supersession, or sequencing", "category": "domain"},
    "trinitarian":      {"definition": "Applies three-part alignment or scoring logic (3OT)", "category": "structural"},
    "input_sanitation": {"definition": "Cleans and validates external inputs before processing", "category": "structural"},
    "governance":       {"definition": "Subject to or enforces project governance contracts", "category": "structural"},
    "deterministic":    {"definition": "Produces the same output for the same input (side-effect-free)", "category": "structural"},
    "stateless":        {"definition": "Holds no persistent internal state between invocations", "category": "structural"},
    "stateful":         {"definition": "Maintains state across invocations or sessions", "category": "structural"},
    "audited":          {"definition": "Subject to audit logging or traceability requirements", "category": "structural"},
    "embedded":         {"definition": "Directly embedded context in the DRAC invariant layer", "category": "structural"},
    "numeric_compute":  {"definition": "Executes arithmetic or algebraic transformations", "category": "domain"},
    "support_services": {"definition": "General-purpose helper with no direct reasoning role", "category": "domain"},
}
...
def _refine_taxonomy(stem: str, source_dir_name: str) -> str:
    ...
    if any(k in s for k in ["valid", "guard", "gate", "constraint", "sanitiz", "3pdn"]):
        return "safety_core"
    if any(k in s for k in ["inference", "logic", "trinitarian", "bijective", "recursion",
                              "evidence", "hypostatic", "necessary_exist"]):
...
def _extract_py_meta(path: Path) -> dict:
    meta = {"functions": [], "classes": [], "docstring": "", "imports": []}
    try:
...
def _infer_tags(stem: str, taxonomy: str, meta: dict, src_dir: str) -> list[str]:
    ...
    if "bootstrap" in s or "bootstrap" in h:    tags.add("bootstrap")
    if "identity" in s or "identity" in h:      tags.add("identity")
    if "bijective" in s or "bijection" in h:    tags.add("bijective")
    if "temporal" in s or "temporal" in h:      tags.add("temporal")
    if "trinitarian" in s:                      tags.add("trinitarian")
    if "sanitiz" in s or "sanitiz" in h:        tags.add("input_sanitation")
    if "probabilis" in h or "bayes" in h:       tags.add("probabilistic")
    if "plan" in h:                             tags.add("planning")
    if "tool" in h:                             tags.add("tool_interface")
    if "learn" in h:                            tags.add("learning")
    if "memory" in h or "catalog" in h:         tags.add("memory_access")
    if "numeric" in h or "math" in h:           tags.add("numeric_compute")
    if "monolith" in s:                         tags.add("protocol")
...
def _compatible_clusters_by_tags(fbc_tags: list[str], clusters: list[dict]) -> list[str]:
...
def run() -> None:
    ...
    for dir_name, py_file, src_dir_name in candidates:
        ...
        fbc_entries.append({
            ...
            "structural_role":      taxonomy.replace("_core", "").replace("_", " ").title(),
        })
    ...
    fbc_entries.append({
        ...
        "classes":            [],
        "dependencies":       [],
        "tags":               ["protocol", "axiomatic", "constraint", "governance", "deterministic"],
        "compatible_clusters": ["agent_runtime", "support_services"],
        "structural_role":    "Protocol",
    })
    taxonomy_dist["protocol_core"] += 1
    print(f"  FBC_{fbc_counter:04d}  CoreConfig_0001                                    → protocol_core")
    ...
    for fbc in fbc_entries:
        ...
        for cl_name in compat_clusters:
            ...
            cl_obj = next((c for c in af_clusters if c["cluster_name"] == cl_name), None)
            if cl_obj:
        ...
        if compat_clusters:
            print(f"  {fbc['core_id']}  {fbc['core_name']:40s} ↔ {', '.join(compat_clusters)}")
    ...
    if uncovered:
        print(f"  NOTE: {len(uncovered)} clusters with no direct core binding: {sorted(uncovered)}")
    else:
    ...
    files = {
        ...
        "af_core_compatibility_matrix.json": {"schema_version": "1.0.0",
                                              "generated_at": datetime.now(timezone.utc).isoformat(),
