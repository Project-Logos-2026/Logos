...
def test_router_determinism():
    ...
    p = pathlib.Path("LOGOS_SYSTEM/TEST_SUITE/ontological_properties.json").read_bytes()
    h1 = hashlib.sha256(p).hexdigest()
    subprocess.run(["python", "tools/audit_and_emit.py", "--write"], check=True)
    h2 = hashlib.sha256(
        pathlib.Path("LOGOS_SYSTEM/TEST_SUITE/ontological_properties.json").read_bytes()
    ).hexdigest()
    assert h1 == h2
