...
def facade_namespace_for_root(root: str) -> str:
    u = root.upper()
    if "EXECUTION" in u:                         return "logos.imports.execution"
    if "OPPERATIONS" in u or "OPERATIONS" in u:  return "logos.imports.operations"
    if "RUNTIME_BRIDGE" in u:                    return "logos.imports.bridge"
    if "SHARED_UTILS" in u or "SHARED" in u:     return "logos.imports.shared"
    if "GOVERNANCE" in u or "_GOVERNANCE" in u:  return "logos.imports.governance"
    if "SYSTEM_STACK" in u or "AGENTS" in u:     return "logos.imports.agents"
    if "STARTUP" in u or "BOOT" in u:            return "logos.imports.startup"
    if "BLUEPRINT" in u:                         return "logos.imports.blueprints"
    if "DOCUMENT" in u:                          return "logos.imports.documents"
...
def step1_load_graph():
    ...
    dep_graph  = require(ARCHON / "runtime_dependency_graph.json",  "runtime_dependency_graph")
    ...
    if py_repo is None:
        py_repo = load(BLUEPRINTS_CIF / "repo_python_files.json",   "repo_python_files(blueprints)")
    if py_repo is None:
        py_repo = []
    ...
    print(f"  [i] Graph nodes: {len(dep_graph['nodes'])}  edges: {len(dep_graph['edges'])}")
...
def step2_load_violations():
    ...
    v1_raw = load(BLUEPRINTS_CIF / "Deep_Import_Violations.json", "Deep_Import_Violations")
    v2     = load(ARCHON / "runtime_deep_import_violations.json", "runtime_deep_import_violations")
    v3     = load(CLUSTER_DIR / "cluster_boundary_violations.json", "cluster_boundary_violations")
...
def step3_load_candidates():
    ...
    c1 = load(ARCHON / "canonical_facade_candidates.json",       "canonical_facade_candidates(archon)")
...
def step5_build_facade_map(
    ...
    for root, mods in sorted(root_modules.items()):
        ...
        if explicit:
            ...
            facade_mod = max(
                mods,
                key=lambda m: in_deg.get(m, 0) + out_deg.get(m, 0),
                default=None,
            )
...
def step7_feasibility(
    ...
    for rule in rewrite_table:
        ...
        if rep.count(".") > 3:
            reasons.append(f"Replacement namespace is still deep ({rep.count('.')} levels)")
            ok = False
        ...
        if src_cluster == tc and tc not in ("", "unknown"):
            reasons.append("Intra-cluster import — facade rewrite optional, not mandatory")
    ...
    print(f"  [i] Feasibility breakdown:")
...
def step9_repair_plan(
    ...
    lines = [
        ...
        "",
        f"**Generated:** {now}",
        f"**Mode:** Analysis Only — No Repository Mutation",
        "",
        "---",
        "",
        "## 1. Summary",
    ...
    for r in auto_rules[:20]:
        src = r["source_module"][-50:] if len(r["source_module"]) > 50 else r["source_module"]
        ill = r["illegal_import"][-55:] if len(r["illegal_import"]) > 55 else r["illegal_import"]
        lines.append(
    ...
    if manual_rules:
        ...
        for r in manual_rules[:30]:
            src = r["source_module"][-50:] if len(r["source_module"]) > 50 else r["source_module"]
            ill = r["illegal_import"][-50:] if len(r["illegal_import"]) > 50 else r["illegal_import"]
